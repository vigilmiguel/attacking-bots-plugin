...
SAMPGDK_NATIVE(bool, WC_DamagePlayer(int playerid, float damage, int issuerid, int weaponid, int bodypart))
{
	static AMX_NATIVE native;
	cell retval;
	cell params[6];
	sampgdk_log_debug("WC_DamagePlayer(%d, %f, %d, %d, %d)", playerid, damage, issuerid, weaponid, bodypart);
	native = sampgdk_native_find_flexible("WC_DamagePlayer", native);
	params[0] = 5 * sizeof(cell);
	params[1] = (cell)playerid;
	params[2] = amx_ftoc(damage);
	params[3] = (cell)issuerid;
	params[4] = (cell)weaponid;
	params[5] = (cell)bodypart;
	retval = native(sampgdk_fakeamx_amx(), params);
	return !!(retval);
}
